...
latches writes exactly as a schematic would. Load a `.bin` built by `zasm`; the
byte at the current address is highlighted as the CPU walks through it.
...
they will appear, alongside the disassembly `zasm` can already produce.
